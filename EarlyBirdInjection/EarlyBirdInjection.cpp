...
bool injectIntoProc(HANDLE hProc, HANDLE hThread) {
...
int wmain(int argc, wchar_t* argv[]) {
	...
	if (!CreateProcessW(L"C:\\Windows\\System32\\calc.exe", NULL, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si, &victimProcInfo)) {
		std::cerr << "Victim process creation failed: " << GetLastError() << '\n';
		return EXIT_FAILURE;
	}

	if (!injectIntoProc(victimProcInfo.hProcess, victimProcInfo.hThread)) {
